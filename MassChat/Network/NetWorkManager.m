...
+(instancetype)shareManager
{
    ...
    dispatch_once(&onceToken, ^{
        ...
        manager = [[self alloc] initWithBaseURL:[NSURL URLWithString:DEFAULT_BASEURL]];
...
+(NSMutableDictionary*)addtionalSysInfo{
        NSDictionary *systemInfos = @{
                                      @"os":@"iOS"
//                                      @"model": [EDQLogin instance].deviceModel?:@"-1",
//                                      @"uuid": [EDQLogin instance].deviceTokenOrAdId?:@"-1",
//                                      @"os":[EDQLogin instance].operationSystemVersion?:@"-1",
//                                @"userid":user_info.employeeUid?:@"-1"
                                      };
        
        return [systemInfos mutableCopy];
}
...
+(void)requestWithType:(HttpRequestType)type withUrlString:(NSString *)urlString withParaments:(id)paraments withSuccessBlock:(requestSuccess)successBlock withFailureBlock:(requestFailure)failureBlock progress:(downloadProgress)progress
{
        ...
        NSMutableDictionary* param = [self addtionalSysInfo];
        [param addEntriesFromDictionary:paraments];
        switch (type) {
                
                case HttpRequestTypeGet:
                {
                        
                        [[NetWorkManager shareManager] GET:urlString parameters:param progress:^(NSProgress * _Nonnull downloadProgress) {
                        ...
                        
                {
                        ...
                        [[NetWorkManager shareManager] POST:urlString parameters:param progress:^(NSProgress * _Nonnull uploadProgress) {
